...
sub ProcessMMD2PDF {
	...
	my $tex_string = "; pdflatex mmd.tex; biber mmd; makeindex -t mmd.glg -o mmd.gls -s mmd.ist mmd.glo; makeindex -s `kpsewhich basic.gst` -o mmd.gls mmd.glo; pdflatex mmd.tex; pdflatex mmd.tex; pdflatex mmd.tex; pdflatex mmd.tex";

	if ($^O =~ /MSWin/) {
		$tex_string = "& pdflatex mmd.tex & biber mmd & makeindex -t mmd.glg -o mmd.gls -s mmd.ist mmd.glo & makeindex -s `kpsewhich basic.gst` -o mmd.gls mmd.glo & pdflatex mmd.tex & pdflatex mmd.tex & pdflatex mmd.tex & pdflatex mmd.tex";
	}	
...
sub ProcessMMD2PDFXeLaTeX {
	...
	my $tex_string = "; xetex_mt mmd.tex; biber -u -U mmd; makeindex -t mmd.glg -o mmd.gls -s mmd.ist mmd.glo; makeindex -s `kpsewhich basic.gst` -o mmd.gls mmd.glo; xetex_mt mmd.tex; xetex_mt mmd.tex; xetex_mt mmd.tex; xetex_mt mmd.tex";

	if ($^O =~ /MSWin/) {
		$tex_string = "& xetex_mt mmd.tex & biber -u -U mmd & makeindex -t mmd.glg -o mmd.gls -s mmd.ist mmd.glo & makeindex -s `kpsewhich basic.gst` -o mmd.gls mmd.glo & xetex_mt mmd.tex & xetex_mt mmd.tex & xetex_mt mmd.tex & xetex_mt mmd.tex";
	}
	PDFEngine($MMDPath, $input_file, $tex_string, $text);
}
